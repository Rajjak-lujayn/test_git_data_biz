			...
			<div class="navbar-identity">
				...
				<a href="{{ url('/') }}" class="navbar-brand logo logo-title">
					<img src="{{ imgUrl(config('settings.app.logo'), 'logo') }}"
						 ...
						 data-toggle="tooltip"
						 data-original-title="{!! isset($logoLabel) ? $logoLabel : '' !!}"/ style="width:160px;height:70px;">
				...
				<ul class="nav navbar-nav ml-auto navbar-right">
					...
					@else
						...
						<li class="nav-item dropdown no-arrow">
							<a href="#" class="dropdown-toggle nav-link dropdown-link-menu" data-toggle="dropdown">
								<i class="icon-user fa hidden-sm"></i>
								<span>{{ auth()->user()->name }}</span>
								@if (isset(auth()->user()->credit_limit) && auth()->user()->credit_limit > 0)
								<span class="badge badge-pill badge-important credit_counter">{{ auth()->user()->credit_limit }}</span>
								@endif
