		...
		foreach ($lr_listing as $list) {
			...
			if (!$listing->isEmpty()) {
				...
				foreach (explode('-', $input) as $key => $info) {
					// dd($info);
					$test = preg_split('/(?<=[0-9])(?=[a-z]+)/i', $info);
					$a = $test[1];
					$no_k = str_replace($a, "", $test[0]);
...
Route::get('update-cron', function () {
	...
	foreach ($lr_listing_update as $value) {
		$aray = [];
		if ($value->Revenue != '') {
			foreach (explode('-', $value->Revenue) as $key => $info) {
				$test = preg_split('/(?<=[0-9])(?=[a-z]+)/i', $info);
				$test_replace = str_replace( array( '\'', '"', ',' , ';', '<', '>' ), ' ', $test);
				$a = $test[1];
				$no_k = str_replace($a, "", $test_replace[0]);
				dd($no_k);
				$dotted = str_replace(",", ".", $no_k);
				if ($a == 'K') {
					$aray[$key] = $dotted * 1000;
